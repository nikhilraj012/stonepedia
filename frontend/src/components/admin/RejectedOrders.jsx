...
const RejectedOrders = () => {
  ...
  useEffect(() => {
    const fetchRejectedOrders = async () => {
      try {
        const rejectedOrdersCollection = collection(db, "rejectedOrders");
        const ordersSnapshot = await getDocs(rejectedOrdersCollection);
        const ordersList = ordersSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        setRejectedOrders(ordersList);
    ...
    <div>
      ...
      <div className="mt-5 bg-white shadow rounded-lg p-5">
        {loading ? (
          ...
          <p className="text-gray-600 text-center text-2xl">No rejected orders found.</p>
        )}
      </div>
    </div>
  );
